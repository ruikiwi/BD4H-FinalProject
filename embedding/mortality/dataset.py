...
class MortalityDataset(DatasetTemplate):
    ...
    def __init__(self):
        ...
        if cfg.data_source == 'mimic3':
            self.dict_file = join(cfg.dataset_dir, 'mimic3_dict _true.json')
            self.patients_codes_file = join(cfg.dataset_dir, 'patients_mimic3_codes_true.json')
            with open(self.patients_codes_file) as read_file:
                ...
                for pa in  self.patients:
                    if pa['Death'] == 1 and death_count < count_limit:
                        death_count += 1
                        new_patients.append(pa)
                    elif pa['Death'] == 0 and live_count < count_limit:
                        live_count += 1
                        new_patients.append(pa)
                    else:
                        continue
            ...
            '''
            with open(self.dict_file) as read_file:
                self.dictionary = json.load(read_file)
            self.reverse_dictionary = {int(k): v for k, v in self.dictionary.items()}
